...
const Footer = () => {
  ...
  const FooterLink = ({ name, url }) => (
    ...
    >
      ...
      <ChevronRight className="h-3 w-3 ml-1 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300 text-blue-600" />
    </a>
  );
  ...
  return (
    <footer className="relative bg-white pt-16 overflow-hidden">
      ...
      {/* Partners Section - Marquee-style feel but static grid */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16">
        ...
        <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12 lg:gap-16">
          {partnerLogos.map((logo, index) => (
            <div
              key={index}
              className="group relative filter grayscale hover:grayscale-0 transition-all duration-500 transform hover:-translate-y-1"
            >
              <img
                src={logo.src}
                alt={logo.alt}
                title={logo.name}
                className="h-9 md:h-11 w-auto object-contain opacity-60 group-hover:opacity-100"
              />
            </div>
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8">
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className="flex flex-col space-y-4">
                <img src={clsdlogo} alt="CLSD Logo" className="h-16 w-auto self-start drop-shadow-sm" />
                <div>
                  <h4 className="text-xl font-bold text-slate-900 tracking-tight leading-tight">
                    Center for Lakes <br />
                    <span className="text-blue-600">Sustainable Development</span>
                  </h4>
                </div>
              </div>
            ...
            <div className="lg:col-span-3">
              <div className="flex items-center space-x-2 mb-6">
                <h4 className="text-xs font-bold text-slate-900 uppercase tracking-widest mb-6 border-l-2 border-blue-500 pl-3">
                  About GOVPH
                </h4>
              </div>
              ...
              <h5 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.15em] mb-3">
                Quick Links
              </h5>
